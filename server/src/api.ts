...
export function createApiFetch(options: ApiOptions = {}): ApiFetch {
  ...
  const VALIDATOR_JAR = options.validatorJarPath ??
  ...
  async function handleTxSearch(request: Request) {
    try {
      ...
      const results = await Promise.all(terms.map(async (q) => {
        try {
          const out = await terminologySearch.searchWithGuidance(q, { systems: body.systems, limit });
          return { query: q, hits: out.hits as TerminologyHit[], count: out.count ?? (out.hits?.length ?? 0), fullSystem: !!(out as any).fullSystem, guidance: (out as any).guidance };
        } catch (e) {
  ...
  async function handleCodesExists(request: Request) {
    try {
      ...
      const results = items.map(it => {
        ...
        try {
          const row = db.query<{ display?: string }>(
  ...
  async function handleValidate(request: Request) {
    try {
      ...
      return Response.json(
        { valid: false, issues: [{ severity: "error", code: "exception", details: String(error) }] },
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
  }
      ...
      const results = await Promise.all(
        body.resources.map(async (item) => {
          try {
            ...
            return { id: item.id, valid: false, issues: [{ severity: "error", code: "exception", details: String(error) }] };
          }
        })
      );
