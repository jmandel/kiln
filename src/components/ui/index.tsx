...
export const Card = ({ children, className = '' }: { children: ReactNode; className?: string }) => (
  <div className={`card-kiln ${className}`}>{children}</div>
);
    ...
    {views.map((view) => (
      <button
        ...
        className={`px-2 py-1 text-xs rounded transition-colors ${
          active === view ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:text-gray-900'
        }`}
...
export const StatusBadge = ({ status }: { status: string }) => {
  ...
  return <span className={variant.className}>{variant.text}</span>;
};
...
export const ErrorBanner = ({
  ...
  <div className="card-kiln bg-rose-50 border-rose-200 text-rose-800 p-4">
    ...
    <div className="flex items-center gap-3 mt-2">
      {onResume && (
        <button className="btn-kiln text-xs px-2.5 py-1" onClick={onResume}>
          Resume
        </button>
      )}
      {onOpenFailed && (
        <button className="btn-kiln-outline text-xs px-2.5 py-1" onClick={onOpenFailed}>
          Open failed step
        </button>
      )}
    </div>
