...
export function ArtifactsTable({ items, onOpen }: ArtifactsTableProps) {
  ...
  const filtered = filterPhase === 'all' 
    ? items 
    : items.filter(i => (i.phase || 'other') === filterPhase);
          ...
          <tbody>
            {sorted.map(item => (
              ...
              >
                ...
                <td className="py-3">
                  <Badge variant={getVariantForKind(item.kind)}>
                    {item.kind}
                  </Badge>
                </td>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {items.map(item => (
        ...
        >
          ...
          {item.notes && (
            <div className="text-xs text-gray-500 mb-2">{item.notes}</div>
          )}
      ...
      {sortedByTime.map((item, idx) => (
        ...
        >
          ...
          <div className="flex-1">
            ...
            {item.notes && (
              <div className="text-xs text-gray-500 mb-1">{item.notes}</div>
            )}
