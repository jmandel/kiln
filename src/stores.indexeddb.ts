  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      // jobs
      {
        ...
        try {
          (s as any).createIndex('dependsOn', 'dependsOn', { unique: false, multiEntry: true });
        } catch {}
      }
      // artifacts
      {
  ...
  const stores: Stores = {
    jobs: {
      async create(id, title, type, inputs, dependsOn) {
        ...
        if (!existing) {
          ...
          const rec = {
            ...
            type,
            inputs,
            status: dependsOn && dependsOn.length ? 'blocked' : 'queued',
            dependsOn: dependsOn || [],
            lastError: null,
            cacheVersion: 0,
            createdAt: now,
      ...
      async all() {
        ...
        const s = t.objectStore('jobs');
        const req = s.getAll();
        const list: any[] = await new Promise((res) => {
      ...
      async upsert(jobRec) {
        ...
        try {
          console.log('[job.upsert]', {
            id: jobRec.id,
            status: (jobRec as any).status,
            runCount: (jobRec as any).runCount,
          });
        } catch {}
        ...
        events.emit({
          ...
          jobId: jobRec.id,
          status: (jobRec as any).status,
          lastError: (jobRec as any).lastError,
        } as any);
      },
      async setDependsOn(id, dependsOn) {
        ...
        if (job) {
          ...
          try {
            console.log('[job.dependsOn]', { id, dependsOn: job.dependsOn });
          } catch {}
          events.emit({
            type: 'job_status',
            jobId: id,
      ...
      async listByDependsOn(parentId) {
        ...
        try {
          ...
          const list: any[] = await new Promise((res) => {
            reqAll.onsuccess = () => res(reqAll.result as any[]);
          });
    ...
    artifacts: {
      async get(id: ID): Promise<Artifact | undefined> {
        ...
        const req = s.get(id);
        return await new Promise((res) => {
          req.onsuccess = () => res(req.result as Artifact | undefined);
        });
      },
      ...
      async listByJob(jobId: ID, pred?: (a: Artifact) => boolean): Promise<Artifact[]> {
        ...
        let list: Artifact[] = await new Promise((res) => {
          (req as any).onsuccess = () => res((req as any).result as Artifact[]);
        });
      ...
      async latestVersion(jobId: ID, kind: string, tagsKey?: string, tagsValue?: any): Promise<number | null> {
        const list = await this.listByJob(
          jobId,
          (a: Artifact) => a.kind === kind && (!tagsKey || a.tags?.[tagsKey] === tagsValue)
        );
      ...
      async deleteByJob(jobId: ID): Promise<void> {
        ...
        const s = t.objectStore('artifacts');
        const req = s.index('jobId').getAll(IDBKeyRange.only(jobId));
        const list: Artifact[] = await new Promise((res) => {
          req.onsuccess = () => res(req.result as Artifact[]);
        });
    ...
    steps: {
      ...
      async listByJob(jobId: ID): Promise<Step[]> {
        ...
        const all: any[] = await new Promise((res) => {
          (req as any).onsuccess = () => res((req as any).result as any[]);
        });
        const steps = all.map((r) => {
          const { pk: _pk, ...rest } = r;
          return rest as Step;
        });
    ...
    links: {
      async get(id: ID): Promise<Link | undefined> {
        ...
        const req = s.get(id);
        return await new Promise((res) => {
          req.onsuccess = () => res(req.result as Link | undefined);
        });
      },
      async upsert(l: Link): Promise<void> {
        ...
        const list: Link[] = await new Promise((res) => {
          reqAll.onsuccess = () => res(reqAll.result as Link[]);
        });
        const composite = `${rec.jobId}-${rec.fromType}-${rec.fromId}-${rec.toType}-${rec.toId}-${rec.role}`;
        const dup = list.find(
          (li) =>
            `${li.jobId}-${li.fromType}-${li.fromId}-${li.toType}-${li.toId}-${li.role}` === composite &&
            li.id !== rec.id
        );
      ...
      async listByJob(jobId: ID): Promise<Link[]> {
        ...
        const req = s.index('jobId').getAll(IDBKeyRange.only(jobId));
        const list: Link[] = await new Promise((res) => {
          (req as any).onsuccess = () => res((req as any).result as Link[]);
        });
      ...
      async deleteByJob(jobId: ID): Promise<void> {
        ...
        const req = s.index('jobId').getAll(IDBKeyRange.only(jobId));
        const list: Link[] = await new Promise((res) => {
          req.onsuccess = () => res(req.result as Link[]);
        });
