...
function definePhase(_name: string, _boundaryTags: Record<string, any>, taskFns: Array<(ctx: Context, params?: any) => Promise<void>>): (ctx: Context, params?: any) => Promise<void> {
  return async (ctx: Context, params?: any) => { for (const fn of taskFns) await fn(ctx, params); };
}
  ...
  return async (ctx: Context, params: any) => {
    ...
    while (attempt <= options.maxRevs) {
      ...
      if (options.approvalThreshold && score < options.approvalThreshold) {
        console.warn(`Low score warning: ${options.title || 'loop'} v${version} scored ${score} (below threshold ${options.approvalThreshold})`);
      }
...
function createPlanOutlineTask(sketch: string) {
  return async (ctx: Context) => {
    await runLLMTask<any>(ctx, 'plan_outline', 'plan_outline', { sketch }, { expect: 'json', tags: { phase: 'planning' }, artifact: { kind: 'NarrativeOutline', version: 1, title: 'Outline v1', tags: { phase: 'planning', responseJson: undefined }, contentType: 'json' } });
  };
}
...
function createSectionDraftTask(section: string, sectionIndex: number) {
  return async (ctx: Context, version: number, { section: sec, outline, sketch }: any) => {
    ...
    await runLLMTask<string>(ctx, 'draft_section', 'draft_section', { section: sec, brief: brief?.content || '', sketch, guidance, priorSummary }, { expect: 'text', tags: { phase: 'sections', section: sec, version }, artifact: { kind: 'SectionDraft', version, title: `Draft ${sec} v${version}`, tags: { section: sec, verb: 'draft' }, links: [ ...(brief ? [{ dir: 'from' as const, role: 'uses', ref: { type: 'artifact' as const, id: brief.id } }] : []) ], contentType: 'text' } });
  };
}
...
function createSectionCritiqueTask(section: string, sectionIndex: number) {
  return async (ctx: Context, version: number, { section: sec, outline, sketch }: any) => {
    ...
    const { result: c, artifactId } = await runLLMTask<any>(ctx, 'critique_section', 'critique_section', { section: sec, draft: draft?.content || '', brief: brief?.content || '', sketch, guidance, priorSummary }, { expect: 'json', tags: { phase: 'sections', section: sec, version }, artifact: { kind: 'SectionCritique', version: 1, title: `Critique ${sec} for v${version}`, tags: { section: sec, draftVersion: version, threshold: TARGETS.SECTION, verb: 'critique', responseJson: undefined }, links: [ ...(draft ? [{ dir: 'from' as const, role: 'critiques', ref: { type: 'artifact' as const, id: draft.id } }] : []) ], contentType: 'json' } });
    if (artifactId) { const art = await ctx.stores.artifacts.get(artifactId); if (art) await ctx.stores.artifacts.upsert({ ...art, tags: { ...(art.tags || {}), score: (c as any).score, responseJson: c } }); }
...
function createNoteCritiqueTask(_initial: number) {
  return async (ctx: Context, ver: number, { outline, sketch }: any) => {
    ...
    const guidance = outline?.guidance || '';
    const sectionSummaries = await getSectionSummaries(ctx, outline);
    const { result: c, artifactId } = await runLLMTask<any>(ctx, 'critique_note', 'critique_note', { noteDraft: note?.content || '', sketch, guidance, sectionSummaries }, { expect: 'json', tags: { phase: 'note_review', version: ver }, artifact: { kind: 'NoteCritique', version: 1, title: `Critique Note v${ver}`, tags: { noteVersion: ver, verb: 'critique' }, links: [ ...(note ? [{ dir: 'from' as const, role: 'critiques', ref: { type: 'artifact' as const, id: note.id } }] : []) ], contentType: 'json' } });
    if (artifactId) { const art = await ctx.stores.artifacts.get(artifactId); if (art) await ctx.stores.artifacts.upsert({ ...art, tags: { ...(art.tags || {}), score: (c as any).score, threshold: getTargets().NOTE, responseJson: c } }); }
    return { score: (c as any).score };
  };
}
...
export function buildNarrativeWorkflow(inputs: NarrativeInputs): DocumentWorkflow<NarrativeInputs> {
  ...
  const planningPhase = definePhase('Planning', { phase: 'planning' }, [ async (ctx) => await planTask(ctx), createRealizeOutlineTask() ]);
  const sectionsPhase = definePhase('Sections', { phase: 'sections' }, [ async (ctx) => {
    if (!(await ctx.isPhaseComplete('planning'))) await planningPhase(ctx, {});
    const outline = await getOutlineFromSteps(ctx);
    const sections = Array.isArray(outline?.sections) ? outline.sections : [];
    if (!sections.length) throw new Error('Outline missing/invalid (no sections). Check phase:planning:plan_outline step for errors.');
    for (let i = 0; i < sections.length; i++) {
      const sec = sections[i];
      const sectionLoop = revisionLoop({ target: TARGETS.SECTION, maxRevs: TARGETS.SECTION_MAX_REVS, approvalThreshold: TARGETS.SECTION * 0.8, getLatestVersion: latestDraftVersion, draftTask: createSectionDraftTask(sec.title, i), critiqueTask: createSectionCritiqueTask(sec.title, i), decideTask: createSectionDecideTask(sec.title), title: `Section: ${sec.title}` });
