...
export async function resumeDocument(stores: Stores, documentId: ID): Promise<void> {
  try {
    console.log('[WF]', JSON.stringify({ ts: new Date().toISOString(), type: 'resume.begin', documentId }));
  } catch {}
  ...
  const pipeline: Array<(ctx: Context) => Promise<void>> = (function () {
    ...
    if (isFhir) {
      return buildFhirWorkflow({
        noteText: (doc.inputs as any).noteText,
        source: (doc.inputs as any).source,
      } as FhirInputs);
    }
    return [];
  })();
  ...
  if (wfIds.length) {
    ...
    try {
      stores.events.emit({ type: 'artifacts_cleared', documentId } as any);
    } catch {}
    try {
      stores.events.emit({ type: 'links_cleared', documentId } as any);
    } catch {}
    for (const wfId of wfIds) {
      ...
      try {
        ...
        try {
          console.error('[WF] resumeDocument error', e);
        } catch {}
      }
    }
    try {
  ...
  if (replayWfId) {
    ...
    try {
      ...
      try {
        console.error('[WF] resumeDocument replay error', e);
      } catch {}
    }
  }
  ...
  if (!replayWfId) {
    ...
    try {
      console.log('[WF] resumeDocument: created and starting new workflow', { documentId, wfId, type: doc.type });
    } catch {}
    try {
      await runWorkflow(stores, wfId, documentId, pipeline, { type: doc.type, inputs: doc.inputs as any });
    } catch (e) {
      try {
        console.error('[WF] resumeDocument start error', e);
      } catch {}
    }
    return;
  }
...
export async function createAndRunDocument<T extends InputsUnion>(
  ...
  if (blockedOn.length > 0) {
    ...
    if (rec) {
      ...
      try {
        console.log('[deps] created blocked doc', { documentId, type, blockedOn });
      } catch {}
    }
  }
  ...
  if (shouldRun) {
    ...
    try {
      console.log('[deps] starting workflow immediately', { documentId, workflowId, type });
    } catch {}
...
export async function triggerReadyDependents(stores: Stores): Promise<void> {
  ...
  const blocked = all.filter(
    (d) => d.status === 'blocked' && Array.isArray((d as any).tags?.blockedOn) && (d as any).tags.blockedOn.length > 0
  );
  try {
    console.log('[deps] scan', {
      total: all.length,
      blocked: blocked.map((b) => ({ id: b.id, type: (b as any).type, blockedOn: (b as any).tags?.blockedOn })),
    });
  } catch {}
  for (const dep of blocked) {
    ...
    if (anyBlocked) {
      try {
        console.log('[deps] skip: parent still blocked', { depId: dep.id, statusByDep });
      } catch {}
      continue;
    }
    const allDone = statusByDep.every((s) => s.status === 'done');
    try {
      console.log('[deps] check', { depId: dep.id, allDone, statusByDep });
    } catch {}
    if (!allDone) continue;
    try {
      ...
      try {
        console.log('[deps] start dependent', { depId: dep.id, wfId, type: dep.type });
      } catch {}
      runWorkflow(stores, wfId, dep.id, pipeline, { type: dep.type, inputs: dep.inputs as any }).catch(console.error);
    } catch (e) {
      try {
        console.error('triggerReadyDependents error', e);
      } catch {}
    }
  }
}
