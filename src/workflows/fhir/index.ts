...
function definePhase(_name: string, _tags: Record<string, any>, fns: Array<(ctx: Context) => Promise<void>>) {
  return async (ctx: Context) => { for (const fn of fns) await fn(ctx); };
}

async function shortHash(seed: string, len = 8): Promise<string> { const h = await sha256(seed); return h.slice(0, len); }
...
function stitchSectionNarratives(compositionPlan: any, noteText: string): any {
  ...
  if (Array.isArray(compositionPlan.section)) {
    ...
    for (const section of compositionPlan.section) {
      ...
      if (looksLikeSub) {
        ...
        try { console.warn('[FHIR][stitch] Dropping subsection and merging entries into parent:', rawTitle); } catch {}
        continue;
      }
    ...
    for (const section of compositionPlan.section) {
      if (section.text?.div) {
        ...
        if (m && m[1]) {
          ...
          if (rendered != null) { section.text.div = rendered; section.text.status = 'additional'; }
          else { section.text.div = `<div xmlns=\"http://www.w3.org/1999/xhtml\"><p>Narrative for section '${title}' not found in source note.</p></div>`; section.text.status = 'additional'; }
        }
      }
    }
  }
  try {
    ...
    if (cid) {
      ...
      if (!current || typeof current !== 'object' || Array.isArray(current)) compositionPlan.identifier = { value: cid };
    }
  } catch {}
...
export function makeFhirEncodingPhase(noteText: string): (ctx: Context) => Promise<void> {
  return async (ctx: Context) => {
    ...
    const sectionTitles: string[] = [];
    {
      ...
      while ((m = rx.exec(note_text)) !== null) { const t = (m[1] || '').trim(); if (t) sectionTitles.push(t); }
    }
    ...
    if (Array.isArray(compositionPlan.section)) {
      for (const section of compositionPlan.section) {
        if (Array.isArray(section.entry)) {
          for (const entry of section.entry) if (entry.reference && entry.display) references.push({ reference: entry.reference, display: entry.display });
        }
      }
    }
    ...
    const subjectDisplay = (typeof compositionPlan?.subject === 'object' && compositionPlan?.subject?.display)
    ...
    const encounterDisplay = (typeof compositionPlan?.encounter === 'object' && compositionPlan?.encounter?.display)
    ...
    const addAuthorRef = (v: any) => {
      if (!v) return;
      if (typeof v === 'string') { authorRefs.push({ reference: v }); return; }
      if (typeof v === 'object' && v.reference) { authorRefs.push({ reference: v.reference, display: v.display }); }
    };
    if (Array.isArray(compositionPlan?.author)) { for (const a of compositionPlan.author) addAuthorRef(a); }
    ...
    try {
      if (finalComposition?.id) {
        if (!finalComposition.identifier || typeof finalComposition.identifier !== 'object' || Array.isArray(finalComposition.identifier)) finalComposition.identifier = { value: finalComposition.id };
    ...
    const bundle = {
      ...
      entry: [ { fullUrl: `${base}/${finalComposition.resourceType || 'Composition'}/${finalComposition.id}`, resource: finalComposition }, ...finalResources.map((r: any) => ({ fullUrl: `${base}/${r.resourceType}/${r.id || ''}`, resource: r })) ]
    } as any;
    (function pruneDeep(node: any): boolean { if (node == null) return true; if (Array.isArray(node)) { for (let i = node.length - 1; i >= 0; i--) { if (pruneDeep(node[i])) node.splice(i, 1); } return node.length === 0; } if (typeof node === 'object') { for (const k of Object.keys(node)) { const v = (node as any)[k]; if (pruneDeep(v)) delete (node as any)[k]; } return Object.keys(node).length === 0; } return false; })(bundle);
...
export function buildFhirWorkflow(inputs: FhirInputs): DocumentWorkflow<FhirInputs> {
  ...
  const noop = definePhase('FHIR Encoding', { phase: 'fhir' }, [ async () => { throw new Error('FHIR inputs missing noteText or source'); } ]);
  return [noop];
}
